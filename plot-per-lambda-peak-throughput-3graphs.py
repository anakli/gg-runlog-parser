...
#python plot-per-lambda-peak-throughput-3graphs.py gg-cmake-s3netstats-1000lambdas-REDO sort100GB-500worker-TXstats-CrailReFlex.txt videoAnalytics_split_video3res4_100batch1_10batch2_stage1_onlyEPHEMdata_monitorafterS3_REDOGOOD_rxstats.txt
...
workers[workers==0] = 1
GG_CAPACITY = .85
ypeak = np.amax(ytotal, axis=0) / scale_y
...
gg  = ax.plot(x, np.transpose(ypeak), label='gg-cmake-650workers', linewidth=3)
...
#ax.yaxis.set_major_formatter(ticks_y)
#gg  = ax.plot(x, np.transpose(ysum), label='gg-cmake-650workers', linewidth=3)
gg  = ax.plot(x, np.transpose(ypeak), label='gg-cmake-650workers', linewidth=3, color='#1f77b4', linestyle='dashed')
...
sort = ax2.plot(x, np.transpose(ypeak), label='Sort100GB-500workers', color='#2ca02c', linewidth=3)
#ax2.set_xlim(0, 30)
#ax2.set_ylim(0, 0.1)
#ax2.set_ylim(0, 50)
ax2.set_xlim(0, 90)
...
#ax.yaxis.set_major_formatter(ticks_y)
#sort = ax2.plot(x, np.transpose(ysum), label='Sort100GB-500workers', color='#2ca02c', linewidth=3)
sort = ax2.plot(x, np.transpose(ypeak), label='Sort100GB-500workers', color='#2ca02c', linewidth=3, linestyle='dashed')
...
sort = ax3.plot(x, np.transpose(ypeak), label='video-analytics-62workers-620workers', color='#ff7f0e', linewidth=3)
ax3.set_xlim(0, 30)
ax3.set_ylim(0, 130)
#a32.set_ylim(0, 0.1)
...
#ax.yaxis.set_major_formatter(ticks_y)
#sort = ax3.plot(x, np.transpose(ysum), label='video-analytics-500workers', color='#ff7f0e', linewidth=3)
sort = ax3.plot(x, np.transpose(ypeak), color='#ff7f0e', linewidth=3, linestyle='dashed')
...
sort = ax3.plot(x, np.transpose(ypeak), color='#ff7f0e', linewidth=3)
ax3.set_xlim(0, 120)
#ax3.set_ylim(0, 1.0)
ax3.set_xlabel("Time (s)")
...
scale_y = 1e6
BYTES_TO_BITS=8
sort = ax3.plot(x, np.transpose(ypeak), label='video-analytics-62workers-620workers', color='#ff7f0e', linewidth=3, linestyle='dashed')
...
rxstats.close()
#plt.show()
fig.text(0.05, 0.5, 'Peak MB/s per Lambda', ha='center', va='center', rotation='vertical')
#fig.tight_layout()
fig.savefig("peak-throughput-per-lambda.pdf")
plt.show()
exit(0)
