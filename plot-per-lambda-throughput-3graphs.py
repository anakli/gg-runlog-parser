...
#python plot-per-lambda-throughput-3graphs.py gg-cmake-s3netstats-1000lambdas-REDO sort100GB-500worker-TXstats-CrailReFlex.txt videoAnalytics_split_video3res4_100batch1_20batch2_stage1_onlyEPHEMdata.txt
...
#ax.yaxis.set_major_formatter(ticks_y)
#sort = ax3.plot(x, np.transpose(ysum), label='video-analytics-500workers', color='#ff7f0e', linewidth=3)
sort = ax3.plot(x, np.transpose(yavg), color='#ff7f0e', linewidth=3, linestyle='dashed')
...
sort = ax3.plot(x, np.transpose(yavg), color='#ff7f0e', linewidth=3)
ax3.set_xlim(0, 40)
#ax3.set_ylim(0, 1.0)
ax3.set_xlabel("Time (s)")
...
scale_y = 1e6
BYTES_TO_BITS=8
sort = ax3.plot(x, np.transpose(yavg), label='video-analytics-62workers-310workers', color='#ff7f0e', linewidth=3, linestyle='dashed')

rxstats.close()
#plt.show()
fig.text(0.05, 0.5, 'Avg. MB/s per Lambda', ha='center', va='center', rotation='vertical')
#fig.tight_layout()
